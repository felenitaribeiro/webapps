...
const input = readFileSync({json.dumps(str(Path(sys.argv[1]).resolve()))});
const buffer = input.buffer.slice(input.byteOffset, input.byteOffset + input.byteLength);
const v = readVolume(buffer);
...
"""
    ...
    reference = ndimage.affine_transform(
        source,
        mapping[:3, :3],
        mapping[:3, 3],
    ...
    if meta["datatypeCode"] in ranges:
        reference = np.clip(
            np.where(reference > 0, np.floor(reference + 0.5), np.ceil(reference - 0.5)),
            *ranges[meta["datatypeCode"]],
        )
    mismatched = int((browser != reference.astype(np.float32)).sum())
    off_diagonal = np.abs(mapping[:3, :3] - np.diag(np.diag(mapping[:3, :3]))).max()
    max_difference = np.abs(browser - reference).max()
    print(f"off-diagonal max {off_diagonal:.4f}")
    print(f"mismatched {mismatched} of {browser.size} voxels, max abs diff {max_difference:.6g}")
