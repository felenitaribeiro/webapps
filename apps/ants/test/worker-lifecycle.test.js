...
for (const outcome of ["resolve", "reject"]) {
  test(`cancelled file read ${outcome} cannot publish into or terminate a subsequent worker`, async () => {
    ...
    vm.runInContext(`let registrationWorker; let cancelRegistration; const log = { log() {} }; ${registrationSource}`, context);
    const read = deferred();
    const oldJob = context.runRegistration({ arrayBuffer: () => read.promise }, { arrayBuffer: async () => new ArrayBuffer(1) }, () => {});
    ...
    const currentJob = context.runRegistration({ arrayBuffer: () => newRead.promise }, { arrayBuffer: async () => new ArrayBuffer(1) }, () => {});
