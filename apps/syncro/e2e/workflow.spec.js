...
test('compact help, standalone commands and result switching remain reachable',async({page})=>{
 ...
 await page.locator('#input').setInputFiles(scan('anatomical.nii'));
 const result=name=>page.locator(`#resultList [data-result="${name}"]`);
 await expect(page.locator('#resultList .nd-stage-label')).toHaveText(['Original acquired image']);
 await expect(result('original').locator('.nd-view-btn')).toHaveClass(/active/);
 ...
 await expect(page.locator('#resultList .nd-stage-label')).toHaveText(['Original acquired image','Synthetic T1','Warped acquired image']);
 await expect(result('warped-original.nii.gz').locator('.nd-view-btn')).toHaveClass(/active/);
 await expect(page.locator('#opacityControl')).toBeVisible();
 await expect(page.locator('#opacity')).toBeEnabled();
 await page.locator('#opacity').fill('65');
 await expect(page.locator('#opacityValue')).toHaveText('65%');
 await result('original').locator('.nd-view-btn').click();
 await expect(page.locator('#viewLabel')).toHaveText('anatomical.nii');
 await expect(page.locator('#opacityControl')).toBeHidden();
 await result('warped-original.nii.gz').locator('.nd-view-btn').click();
